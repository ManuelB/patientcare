...
(function(JMAP) {
    ...
    const auth = new Obj({
        ...
        fetchSession: function(authenticationUrl, accessToken, username, password) {
            ...
            new HttpRequest({
                ...
                timeout: 45000,
                responseType: 'json',
                data: JSON.stringify({ "username": username, "clientName": "Patient Care JMAP JS Client", "clientVersion": "1.0-SNAPSHOT", "deviceName": "Browser" }),
                onSuccess: function(event) {
                    new HttpRequest({
                        ...
                        timeout: 45000,
                        responseType: 'json',
                        data: JSON.stringify({ "token": event.data.continuationToken, "method": "password", "password": password }),
                        onSuccess: function(event2) {
                            ...
                            var oSessionData = {
                                ...
                                "downloadUrl": "http://localhost/download/{accountId}/{blobId}/{name}?accept={type}",
                            ...
                            for (let sProperty in event2.data) {
                                oSessionData[sProperty] = event2.data[sProperty];
                            }

                            auth.set('authenticationUrl', authenticationUrl)
                                .set('accessToken', event2.data.accessToken)
                                .didAuthenticate(oSessionData);
    ...
    const Connection = Class({
        ...
        */
        ...
        fetchType: function(typeId, accountId, ids) {
            this.callMethod(typeId == "Mailbox" ? 'getMailboxes' : typeId + '/get', {
                /*accountId: accountId,
                ids: ids,*/
            });
        },
...
(function(JMAP) {
    ...
    const iterate = function(fromDate,
        ...
        var maxAttempts =
            ...
            /* MINUTELY || SECONDLY */
            120;
        var useFastPath = !byDay && !byMonthDay && !byMonth && !byYearDay && !byWeekNo;
...
(function(JMAP, undefined) {
    ...
    const CalendarEvent = Class({
        ...
        isInvitation: function() {
            ...
            return !!(participants && (!participantId || !isOwner(participants[participantId])));
            ...
            if (date) {
                ...
                if (utcStart > utcEnd) {
                    if (isAllDay ||
                        ...
                        this.set('utcStart', new Date(+utcStart + (utcEnd - this.get('utcEnd'))));
                    } else {
                        utcEnd.add(1, 'day');
...
(function(JMAP) {
    ...
    const Mailbox = Class({
        ...
        moveTo: function(dest, where) {
            ...
            if (nextSortOrder - prevSortOrder < 2) {
                for (i = 0, p = 32, l = siblings.get('length'); i < l; i += 1, p += 32) {
    ...
    connection.handle(Mailbox, {
        ...
        refresh: function(accountId, ids, state) {
            var get = 'getMailboxes'; // 'Mailbox/get';
            if (ids) {
                this.callMethod(get, {
                    accountId: accountId,
                    ids: ids,
        ...
        'mailboxes' /*Mailbox/get'*/: function(args, _, reqArgs) {
            const isAll = (reqArgs.ids === null);
    ...
    MessageSubmission.makeEnvelope = function(message, extraRecipients) {
        ...
        var mailFrom = {
            email: sender && sender[0] ?
                sender[0].email : message.get('fromEmail'),
            parameters: null,
        };
        var rcptTo = [];
        var seen = {};
        ...
        added = addedMessages.reduce(function(added, message) {
            added.push({
                ...
                threadSK: collapseThreads ?
                    message.getFromPath('thread.storeKey') : null,
